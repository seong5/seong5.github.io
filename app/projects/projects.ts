...
export const projects: Project[] = [
  {
    ...
    work: [
      {
        tag: 'PRODUCT',
        items: [
          '전체 흐름 설계 — 날짜·장소 후보 등록 → 좋아요/싫어요 투표 → 마감 확정 → 캘린더 일정 생성 → 푸시 알림까지 한 흐름으로 이어지는 그룹 의사결정 앱을 기획부터 출시 준비까지 혼자 진행했습니다.',
      ...
      },
      {
        ...
        items: [
          ...
          '중간 상태 제거 — 확정·일정 생성·알림 적재를 같은 트랜잭션에 묶어 확정은 됐는데 일정이 없는 중간 상태가 생기지 않게 했습니다.',
      ...
      },
      {
        ...
        items: [
          ...
          '무한 재귀 회피 — RLS 정책 안에서 group_members를 다시 조회하면 무한 재귀가 나므로, 멤버십 판정을 전부 SECURITY DEFINER 헬퍼 함수 하나로 통일했습니다.',
        ],
      },
      {
        tag: 'OPERATION',
        items: [
          ...
          '중복 발송 차단 — 알림은 큐 테이블과 부분 유니크 인덱스로 중복 발송을 DB 차원에서 막았습니다. 새 후보 알림만 담은 개수만큼 나가야 해서 인덱스 조건에 종류를 적어 예외로 뒀습니다.',
        ],
      },
    ],
    metrics: [
    ...
    gallery: [
      ...
      },
      {
        ...
        h: 1387,
        alt: '다정해 마감 임박 화면',
        caption: '마감 임박 — 남은 시간이 줄면 헤더가 바뀌고, 마감 시각에는 앱이 꺼져 있어도 결과가 확정됩니다.',
      },
    ],
    troubleshooting: [
      {
    ...
    insights: [
      {
        ...
        steps: [
          ...
          {
            title: '0행은 오류가 아니다 — 경합이 가능한 자리',
            points: [
              '후계자를 고르는 조회와 승격 사이에 그 후계자가 탈퇴하면 갱신이 0행을 고치고 성공합니다. 결과는 방장 없는 모임이고 앱 안에 되살릴 방법이 없습니다. 대상 행을 잠그고 갱신 행 수를 확인하는 것으로 막았습니다.',
              '잠금을 여러 개 잡을 때는 순서를 고정합니다. 두 사람이 같은 두 모임을 반대 순서로 잠그면 교착이 납니다.',
              '롤백은 막으려던 기록까지 함께 지웁니다. 초대코드 상한이 무차별 대입만 못 막던 이유가 그것이었습니다 — 정상적인 거절은 예외가 아니라 반환값으로 표현하고, 통과 여부 판단은 호출한 쪽이 합니다.',
            ],
          },
          {
            title: '순서를 보장한다고 착각한 자리',
            points: [
              'SQL 의 AND 는 평가 순서를 보장하지 않습니다. 분당 한도에 이미 걸린 요청이 하루치 쿼터까지 태울 수 있어서 IF 로 분리했습니다.',
              '삽입 직전 트리거는 ON CONFLICT 의 충돌 판정보다 먼저 돕니다. 그래서 정원이 찬 모임에 기존 멤버가 초대코드를 다시 넣으면 아무것도 바뀌지 않는데도 정원이 다 찼어요가 뜨는 문제가 있어 가드를 따로 뒀습니다.',
            ],
          },
          {
          ...
          },
          {
            ...
            points: [
              ...
              '.gitignore 대상 파일은 git status에 뜨지 않으므로 find로 따로 훑는 명령을 절차에 넣었습니다.',
      ...
      },
      {
        ...
        steps: [
          ...
          },
          {
            ...
            points: [
              '개별 재시도 헬퍼를 남겨 두면 새 코드마다 이걸로 감싸야 하나 하는 판단이 계속 남습니다. 전역 처리의 요점이 바로 그 판단을 없애는 것입니다.',
            ],
          },
        ],
      },
      ...
      {
        title: '알림 중복은 앱이 아니라 DB 부분 유니크 인덱스로 막는다',
        steps: [
          {
      ...
      },
      {
        ...
        steps: [
          ...
          },
          {
            title: '지금이라서 안전한 조치',
            points: [
              '카카오 프로필 사진 주소를 https 로 바꾸는 백필은 출시 전이라 안전했습니다. 사진을 일부러 지운 사람이 아직 없어서, 지금 비어 있는 것은 전부 트리거가 놓친 것이지 사용자의 선택이 아니기 때문입니다.',
              '출시 후에 같은 갱신을 돌리면 지운 사람의 사진이 되살아난다고 주석에 적어 뒀습니다. 로그인마다 동기화하지 않는 이유도 같습니다.',
            ],
          },
          {
          ...
          {
            title: '틀린 것도 기록으로 남긴다',
            points: [
              '아무것도 막지 못했던 초대코드 상한 마이그레이션을 되돌리지 않고 그대로 뒀습니다. 다음 마이그레이션이 고쳐 쓰고, 앞 파일 머리에 여기 그대로 두면 상한이 아무것도 막지 못한다, 이유는 그 파일 머리말에 있다고 적었습니다.',
              '대가를 못 박는 문장도 씁니다. 로그인 창을 매번 새로 여는 옵션에는 사파리에 로그인돼 있는데 또 물어보네로 되돌리지 말 것, 그게 이 옵션의 대가다라고 남겼습니다.',
            ],
          },
        ],
      },
