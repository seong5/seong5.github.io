...
export const projects: Project[] = [
  {
    ...
    retrospect: [
      '프로젝트를 진행하며 가장 무서웠던 건 에러가 나지 않는 버그였습니다. 출시 전에 권한을 점검하다 취약점을 4건 찾았는데, 그때까지 앱은 한 번도 이상하게 동작한 적이 없었습니다. 혼자 만들다 보니 내 코드를 의심 또는 검수해 줄 사람이 나밖에 없다는 걸 뼈저리게 느끼게 되었습니다.',
      'App Store 첫 제출시 추가 요청을 받게 되는 것도 처음 겪는 일이었습니다. 출시 이후 앱을 알리는 과정 또한 쉽지 않다는 점 역시 깊이 체감하게 되었습니다. 아무리 잘 만든 서비스라도 알려지지 않으면 아무런 소용이 없다라는 생각을 많이 하게 된 프로젝트입니다.',
      '특정 포지션에만 국한되지 않고 출시하기까지의 모든 상황들을 고려하는 능력을 길러야겠다고 생각하게 된 프로젝트입니다',
    ],
  },
  {
    slug: 'claude-log',
    ...
    retrospect: [
      ...
      '호출을 줄이려고 넣은 가드가 정상적인 재호출까지 막아 버린 적도 있습니다. 무언가를 막는 장치를 넣을 때는 무엇을 통과시켜야 하는지도 같이 정해야 한다는 걸 알게되었습니다.',
      '제가 일상생활에서의 불편함을 직접 개선해 본 첫번째 프로젝트이며 PRD 작성 시 프로젝트의 범위와 기능을 명확히 정의하는 것에 초점을 더 많이 두어야 한다는 점을 깨닫게 되었습니다.',
    ],
  },
  {
    slug: 'umust-erp',
    ...
    retrospect: [
      ...
      '그러면서 ERD와 PRD가 왜 먼저인지를 체감했습니다. 데이터 구조와 요구사항이 정리되기 전에 화면부터 손대면, 잘못 만든 걸 되돌리는 시간이 만드는 시간보다 길어지게 되었습니다.',
      '이 프로젝트를 진행하며 데이터구조와 개발팀, 실사용자인 연구팀과의 협업의 중요성을 다시 한 번 체감했습니다.',
    ],
  },
  {
    slug: 'dobong-admin',
    ...
    retrospect: [
      '도봉라이프라는 앱은 개발된지 2년 정도 된 서비스라고 인수인계 받게 되었고 어드민 콘솔의 부재로 마케팅을 시작하기 않았다고 들었습니다',
      '그러한 이유로 어드민 콘솔을 빠르게 앱과 연동하여 서비스 마케팅을 시작하는 것을 목표로 진행하였습니다. 기획과 디자인을 사전에 정의하지 않고 빠른 프로토타입을 개발한 후 스크럼에서의 피드백을 통해 디벨롭하는 싸이클을 반복적으로 수행했습니다.',
      '프론트2인,백엔드1인이었기에 백엔드 포지션 개발속도가 더뎌지는 상황도 있었지만 POSTMAN Mock Server를 접하게 되어 전체 개발공수를 줄이는 것에 기여했던 프로젝트입니다.',
    ],
  },
  {
    slug: 'deckly',
    ...
    retrospect: [
      '첫번째 인턴이며 TF소속으로 참여하게 되었습니다. 프로젝트 기획/디자인/개발/배포를 혼자서 모두 진행해야했기 때문에 부담이 컸던 기억이 있습니다.',
      '온보딩 시 해소하지 못했던 궁금증들을 프로토타입을 개발하며 피드백을 통해 해소하며 개선하는 방식으로 진행됐습니다. 함께 프로젝트를 진행하는 팀원이 없다는 것이 아쉬웠고 협업이 없는 프로젝트이다보니 개발 과정에서 기술적 해결을 빠르게 해소하기 어려웠습니다.',
      '혼자 프로젝트를 진행하는만큼 자율성을 부여받았지만 그에 따른 책임과 부담 또한 무겁다는 것을 깨닫게 되었습니다.',
    ],
  },
  {
    slug: 'sub-fc',
    ...
    retrospect: [
      '팀원들의 장난스러운 요청으로 시작하게 되었고 그래서 그만큼 더 잘 만들고 싶다는 욕심이 들었던 기억이 있습니다. 실제 배포까지 하여 경기 종료 후 서비스 사용의 플로우까지 모두 경험했고 실사용자가 보장된 서비스라는 점이 소중했던 경험이었습니다.',
      '웹서비스이지만 모바일 환경을 메인으로 개발했으며 서비스 전체 UI개선 작업까지 진행했습니다. 사용자 피드백을 최초로 받게된 프로젝트이며 개선사항의 우선순위를 직접 설정하고 하나씩 반영하며 개선에 대한 기준과 능력을 향상시킬 수 있었습니다.',
    ],
  },
  {
    slug: 'globalnomad',
    ...
    retrospect: [
      ...
      '코드리뷰도 이 프로젝트에서 처음 진행하게 되었고 특별한 경험이었던 기억이 있습니다. API타입을 지정하는 R&R 기간을 따로 정하여 공수기간 손해를 보긴 했지만 다행히 소통을 통해 컴포넌트 및 페이지 R&R을 참고하여 필요한 부분의 API만 건드리기로 합의하여 해결한 점이 인상깊었습니다.',
    ],
  },
  {
    slug: 'whyne',
    ...
    retrospect: [
      '프로젝트를 진행하며 배포 환경을 아무도 제대로 모르고 있었다는 게 드러난 적이 있었습니다. 인스턴스를 재시작했더니 접속이 끊겼는데 어디를 봐야 하는지 적어 둔 곳이 없어서 복구까지 오래 걸렸습니다. 그 뒤로는 한 번 겪은 문제점은 반드시 문서로 짧게라도 남겨 두고 다시 정리하게 되었습니다.',
      '프로젝트 기획 중 팀원 한명이 부트캠프를 종료하게 되어 R&R이 꼬였던 경험도 있고 다른 팀원의 UI컴포넌트 공수가 길어져 스프린트 기간에 맞추지 못하게 되어 제가 팀원의 컴포넌트를 대신 개발하여 공수기간에 맞췄던 기억도 있습니다. UI라이브러리를 사용하지 않고 직접 개발하는 것을 목표로 했던 프로젝트였고 완수할 수 있었습니다.',
    ],
  },
];
