...
export const projects: Project[] = [
  {
    ...
    troubleshooting: [
      ...
      },
      {
        ...
        task:
          "1회 동작이 내부적으로 여러 중복 호출로 처리돼 오류가 나는 원인을 찾아, 정상 흐름에서는 오류가 나지 않게 하는 것이 목표였습니다.",
        action: [
      ...
      },
      {
        ...
        task:
          "파일별로 읽은 바이트 위치(offset)를 기억해 변경 시 추가분만 파싱하도록 — O(전체 파일)을 O(추가된 양)으로 바꾸는 것이 목표였습니다.",
        action: [
    ...
    troubleshooting: [
      {
        ...
        task:
          "로컬은 되고 배포만 안 되는 환경 차이를 역추적해, 프론트 요청 경로와 게이트웨이(Nginx)·백엔드 라우팅을 하나의 규칙으로 정합화하는 것이 목표였습니다.",
      ...
      },
      {
        ...
        task:
          "Swagger는 되는데 앱만 안 되는 차이를 역추적해 문제 레이어(네트워크 / 브라우저 보안정책 / 서버 origin)를 하나씩 분리하고, dev·prod를 안전하게 정합화하는 것이 목표였습니다.",
        action: [
    ...
    troubleshooting: [
      {
        ...
        task:
          "DB 변화에 반응하는 방식 대신, 사용자가 등록 버튼을 눌렀을 때만 작동하는 이벤트 기반으로 알림 시점의 정확도를 높이는 것이 목표였습니다.",
        action: [
    ...
    troubleshooting: [
      {
        ...
        task:
          "하나의 캘린더 컴포넌트가 모든 디바이스의 UI·로직을 처리해 조건문·검증이 기기별로 얽힌 강결합을 풀어, 디바이스별 예약 플로우를 안정적으로 구현하는 것이 목표였습니다.",
        action: [
