...
export type Project = {
   ...
   *
   * 이 파일에서 유일하게 ~습니다 규약이 적용되지 않는 자리다. "프로젝트를 진행하며"로
   * 열고 편안한 말투로 이어 쓴다 — 항목을 나열하는 글이 아니라는 신호를 도입부가 낸다.
   *
...
export const projects: Project[] = [
  {
    ...
    retrospect: [
      '프로젝트를 진행하며 가장 무서웠던 건 에러가 나지 않는 버그였다. 출시 전에 권한을 점검하다 취약점을 네 건 찾았는데, 그때까지 앱은 한 번도 이상하게 동작한 적이 없었다. 혼자 만들다 보니 내 코드를 의심해 줄 사람이 나밖에 없다는 걸 늦게 알았다.',
      'App Store 첫 제출이 반려된 것도 처음 겪는 일이었다. 기능을 다 만들면 끝이라고 생각했는데 심사자가 무엇을 보는지는 따로 알아야 하는 영역이었다. 다음에는 출시 요건을 기능만큼 일찍 확인하려고 한다.',
    ],
  },
  {
    slug: 'claude-log',
    ...
    retrospect: [
      '프로젝트를 진행하며 기능을 만드는 시간보다 그 기능이 가능한지 알아내는 시간이 더 길었다. 과금 없이 사용량을 읽을 방법이 없어 보였고, 쓸 수 있는 토큰을 찾기까지 여러 번 막혔다. 되는지 안 되는지부터 확인하고 시작했어야 했다.',
      '호출을 줄이려고 넣은 가드가 정상적인 재호출까지 막아 버린 적도 있다. 무언가를 막는 장치를 넣을 때는 무엇을 통과시켜야 하는지도 같이 정해야 한다는 걸 그때 알았다.',
    ],
  },
  {
    slug: 'umust-erp',
    ...
    retrospect: [
      '프로젝트를 진행하며 가장 오래 걸린 건 기능을 만드는 일이 아니라, 실사용자에게 맞는 기능이 무엇인지 알아내는 일이었다. 상태 전환을 단계별로 제한해 두었다가 실제 업무에서는 자유롭게 오간다는 걸 뒤늦게 확인하고 걷어낸 적도 있다.',
      '그러면서 ERD와 PRD가 왜 먼저인지를 체감했다. 데이터 구조와 요구사항이 정리되기 전에 화면부터 손대면, 잘못 만든 걸 되돌리는 시간이 만드는 시간보다 길어진다. 다음에는 여기서부터 시작하려고 한다.',
    ],
  },
  {
    slug: 'dobong-admin',
    ...
    retrospect: [
      '프로젝트를 진행하며 프론트와 백엔드 사이의 경계를 정하는 일에 품이 가장 많이 들었다. 응답이 예상과 다를 때마다 화면에서 막을지 서버에서 고칠지를 그때그때 상의해야 했고, 기준을 미리 정해 두지 않은 탓에 같은 논의를 여러 번 반복했다.',
      '390줄짜리 폼 컴포넌트를 나누는 일도 오래 걸렸다. 기능을 다 붙이고 나서 정리하려니 어디까지가 한 덩어리인지 가려내는 데만 시간이 들었다. 급할수록 구조를 먼저 잡는 게 빠르다는 걸 배웠다.',
    ],
  },
  {
    slug: 'deckly',
    ...
    retrospect: [
      '프로젝트를 진행하며 결과가 매번 달라지는 기능을 다루는 게 가장 어려웠다. AI 응답은 같은 입력에도 형태가 흔들려서, 잘 나오는 경우를 기준으로 만들면 금방 무너졌다. 실패를 예외가 아니라 기본값으로 놓고 설계하는 쪽으로 생각을 바꿔야 했다.',
      '느린 화면을 고칠 때 원인부터 짐작한 것은 아쉽다. 응답 크기를 먼저 재 봤다면 목록에 필요 없는 필드까지 받아오고 있다는 걸 훨씬 빨리 알았을 것이다. 다음에는 고치기 전에 재는 순서를 지키려고 한다.',
    ],
  },
  {
    slug: 'sub-fc',
    ...
    retrospect: [
      '프로젝트를 진행하며 쓰는 사람이 옆에 있다는 게 어떤 의미인지 알게 됐다. 팀원 스무 명이 실제로 쓰다 보니 요청이 꾸준히 들어왔는데, 혼자 만들다 보니 받은 순서대로 처리하는 것 말고는 방법이 없었다. 무엇을 먼저 할지 정하는 기준을 세워 두지 않은 게 아쉽다.',
      '지금은 팀 사정으로 서비스가 멈춰 있다. 만드는 동안에는 기능을 얼마나 잘 만드느냐만 생각했는데, 서비스가 계속 쓰이려면 그 바깥의 조건도 함께 봐야 한다는 걸 뒤늦게 알았다.',
    ],
  },
  {
    slug: 'globalnomad',
    ...
    retrospect: [
      '프로젝트를 진행하며 다섯 명이 각자 만든 화면을 합칠 때 문제가 드러났다. 같은 데이터를 네 번 따로 받아오고 있었는데, 각자 맡은 화면만 보면 아무 이상이 없었다. 무엇을 공유하고 무엇을 각자 가질지 먼저 합의했어야 했다.',
      '기기마다 다른 예약 흐름을 만드는 일도 만만치 않았다. 조건이 하나 늘 때마다 분기가 여기저기 퍼져서, 나중에는 화면을 고치는 것보다 어디를 고쳐야 하는지 찾는 데 시간이 더 들었다.',
    ],
  },
  {
    slug: 'whyne',
    ...
    retrospect: [
      '프로젝트를 진행하며 배포 환경을 아무도 제대로 모르고 있었다는 게 드러난 적이 있다. 인스턴스를 재시작했더니 접속이 끊겼는데 어디를 봐야 하는지 적어 둔 곳이 없어서 복구까지 오래 걸렸다. 그 뒤로는 한 번 겪은 절차는 반드시 남겨 두게 됐다.',
      '요청이 두 번 나가거나 같은 필터가 두 번 그려지는 자리도 뒤늦게 발견했다. 화면이 멀쩡히 동작하니 아무도 들여다보지 않았던 곳이다. 눈에 보이는 게 다가 아니라는 걸 팀으로 일하면서 더 자주 느꼈다.',
    ],
  },
];
