...
export type Project = {
  ...
  /**
   * 회고 — 페이지를 닫는 짧은 소회. 한 항목이 한 문단이다.
   *
   * lesson(문제 하나짜리 기술 교훈)·insights(주제 하나짜리 설계 논증)와 층위가 다르다.
   * 여기는 기술이 아니라 과정을 쓴다. 기술로 넘어가면 lesson을 크게 다시 하는 셈이다.
   *
   * 라벨 없는 산문이고 어미는 본문과 같은 ~습니다다. 도입부는 고정하지 않는다 —
   * 프로젝트마다 먼저 꺼낼 이야기가 달라서, 틀을 맞추면 겪은 일이 틀에 끌려간다.
   *
   * 문단을 배열로 끊는 이유는 본문 어디서나 같다 — 문자열에 \n을 박으면
   * 뷰포트마다 조각 줄이 생긴다.
   */
  retrospect?: string[];
};
...
export const projects: Project[] = [
  {
    ...
    summary:
      '모임 날짜·장소를 후보 등록과 투표로 정하는 그룹 약속 앱. 단톡방에서 흐지부지되던 약속을 끝까지 정하도록 기획부터 App Store 출시까지 1인 개발로 진행.',
    detail:
    ...
    resumeBullets: [
      '마감 판정을 PostgreSQL 함수·pg_cron에 두어 앱을 켜지 않아도 약속 자동 확정',
      'RLS·컬럼 단위 권한으로 그룹 간 데이터 경계와 수정 가능 범위 분리',
      'App Store 추가 자료 요청에 신고·차단·금칙어를 구현해 대응하고 출시',
    ],
    ...
    troubleshooting: [
      ...
      },
      {
        ...
        conclusion:
          '다음 방장을 고르는 조회와 승격을 같은 잠금 안에 묶어, 두 사람이 동시에 탈퇴해도 모임에 방장이 반드시 남게 됐습니다.',
        cause:
          '다음 방장을 고르는 SELECT 와 승격 UPDATE 사이에 그 사람이 자기 탈퇴를 커밋하면, 소속 행이 cascade 로 사라져 UPDATE 가 0행을 갱신하고 성공으로 끝납니다. PostgreSQL 에서 갱신된 행이 0개인 것은 오류가 아니기 때문입니다.',
        problem:
        ...
        actions: [
          '다음 방장을 고를 때 대상 행을 SELECT ... FOR UPDATE 로 잠그고, 승격까지 같은 트랜잭션에서 처리하도록 바꿨습니다. 함수 하나가 한 트랜잭션인 것은 맞지만 원자성은 내 트랜잭션 안에서만 지켜지고, READ COMMITTED 에서는 남이 중간에 커밋한 것이 그대로 보입니다.',
    ...
    retrospect: [
      '프로젝트를 진행하며 가장 무서웠던 건 에러가 나지 않는 버그였습니다. 출시 전에 권한을 점검하다 취약점을 4건 찾았는데, 그때까지 앱은 한 번도 이상하게 동작한 적이 없었습니다. 혼자 만들다 보니 제 코드를 의심하고 검수해 줄 사람이 저밖에 없다는 걸 뼈저리게 느꼈습니다.',
      'App Store 첫 제출 시 추가 자료 요청을 받은 것도 처음 겪는 일이었습니다. 출시 이후 앱을 알리는 과정 또한 쉽지 않다는 점을 깊이 체감했습니다. 아무리 잘 만든 서비스라도 알려지지 않으면 아무 소용이 없다는 생각을 많이 한 프로젝트입니다.',
      '특정 포지션에만 국한되지 않고 출시까지의 모든 상황을 고려하는 능력을 길러야겠다고 생각했습니다.',
    ],
  },
  {
    slug: 'claude-log',
    ...
    retrospect: [
      '프로젝트를 진행하며 기능을 만드는 시간보다 그 기능이 가능한지 알아내는 시간이 더 길었습니다. 과금 없이 사용량을 읽을 방법이 없어 보였고, 쓸 수 있는 토큰을 찾기까지 여러 번 막혔습니다.',
      '호출을 줄이려고 넣은 가드가 정상적인 재호출까지 막아 버린 적도 있습니다. 무언가를 막는 장치를 넣을 때는 무엇을 통과시켜야 하는지도 같이 정해야 한다는 걸 알게 되었습니다.',
      '제가 일상에서 겪은 불편함을 직접 개선해 본 첫 번째 프로젝트입니다. PRD를 쓸 때 프로젝트의 범위와 기능을 명확히 정의하는 데 더 초점을 두어야 한다는 점도 깨달았습니다.',
    ],
  },
  {
    slug: 'umust-erp',
    ...
    retrospect: [
      '프로젝트를 진행하며 가장 오래 걸린 건 기능을 만드는 일이 아니라, 실사용자에게 맞는 기능이 무엇인지 협의하며 알아내는 일이었습니다. 상태 전환을 단계별로 제한해 두었다가 실제 업무에서는 자유롭게 오간다는 걸 뒤늦게 확인하고 걷어낸 적도 있습니다.',
      '그러면서 ERD와 PRD가 왜 먼저인지를 체감했습니다. 데이터 구조와 요구사항이 정리되기 전에 화면부터 손대면, 잘못 만든 걸 되돌리는 시간이 만드는 시간보다 길어집니다.',
      '팀에 시니어 개발자도 PM도 없어 제 판단이 맞는지 확인해 줄 사람이 없다는 점이 가장 부담스러웠습니다. 혼자 결론을 내지 않으려고 스크럼 주기를 짧게 가져갔고, 애매한 지점은 쌓아 두지 않고 그때그때 물어 확인받았습니다. 검토해 줄 사람이 없으면 검토받을 자리를 제가 만들어야 한다는 걸 배웠습니다.',
      '이 프로젝트를 진행하며 데이터 구조를 먼저 잡는 일과, 개발팀·실사용자인 연구팀과 협업하는 일의 중요성을 다시 확인했습니다.',
    ],
  },
  {
    slug: 'dobong-admin',
    ...
    retrospect: [
      '도봉라이프는 개발된 지 2년 정도 된 앱이고, 어드민 콘솔이 없어 아직 마케팅을 시작하지 못했다고 인수인계받았습니다.',
      '그래서 어드민 콘솔을 빠르게 앱과 연동해 서비스 마케팅을 시작하는 것을 목표로 잡았습니다. 기획과 디자인을 미리 정의하지 않고 프로토타입을 먼저 만든 뒤 스크럼에서 받은 피드백으로 다듬는 사이클을 반복했습니다.',
      '기획과 디자인을 미리 정의하지 않은 데에는 그럴 수밖에 없던 이유도 있었습니다. 기획을 잡아 줄 PM도, 구조를 검토해 줄 시니어 개발자도 없어서 무엇이 맞는지 판단할 기준이 제 안에만 있었습니다. 대신 스크럼에서 그날 만든 화면을 그대로 열어 보여 주고 어색한 곳을 짚어 달라고 요청하면서, 제 판단이 틀렸을 때 하루 안에 드러나도록 만들었습니다.',
      '프론트 2인, 백엔드 1인이라 백엔드 개발 속도가 더뎌지는 상황도 있었지만, Postman Mock Server를 쓰면서 전체 개발 공수를 줄이는 데 기여한 프로젝트입니다.',
    ],
  },
  {
    slug: 'deckly',
    ...
    retrospect: [
      '첫 번째 인턴이었고 TF 소속으로 참여했습니다. 프로젝트 기획·디자인·개발·배포를 혼자서 모두 진행해야 했기 때문에 부담이 컸던 기억이 있습니다.',
      '온보딩 때 풀지 못한 궁금증은 프로토타입을 만들어 피드백을 받으며 하나씩 해소했습니다. 함께 진행하는 팀원이 없다는 점이 아쉬웠고, 협업이 없다 보니 개발 과정에서 막힌 기술적 문제를 빠르게 풀기 어려웠습니다.',
      '혼자 프로젝트를 진행하는 만큼 자율성을 크게 받았지만, 그에 따르는 책임과 부담 또한 무겁다는 것을 깨달았습니다.',
    ],
  },
  {
    slug: 'sub-fc',
    ...
    retrospect: [
      '팀원들의 장난스러운 요청으로 시작했지만, 그만큼 더 잘 만들고 싶다는 욕심이 들었던 기억이 있습니다. 실제 배포까지 해서 경기가 끝난 뒤 서비스를 쓰는 흐름까지 모두 겪었고, 실사용자가 보장된 서비스였다는 점이 특히 소중했습니다.',
      '웹 서비스이지만 모바일 환경을 기준으로 개발했고, 서비스 전체 UI 개선 작업까지 진행했습니다. 사용자 피드백을 처음 받아 본 프로젝트이기도 한데, 개선 사항의 우선순위를 직접 정하고 하나씩 반영하면서 무엇을 먼저 고쳐야 하는지 판단하는 기준이 생겼습니다.',
    ],
  },
  {
    slug: 'globalnomad',
    ...
    retrospect: [
      '프로젝트를 진행하며 다섯 명이 각자 만든 화면을 합칠 때 문제가 드러났습니다. 같은 데이터를 네 번 따로 받아오고 있었는데, 각자 맡은 화면만 보면 아무 이상이 없었습니다. 무엇을 공유하고 무엇을 각자 가질지 먼저 상세히 합의했어야 했습니다.',
      '코드 리뷰도 이 프로젝트에서 처음 해 봤는데 특별한 경험으로 기억에 남습니다. API 타입을 지정하는 R&R 기간을 따로 잡느라 공수 손해를 보기도 했지만, 컴포넌트·페이지 R&R을 기준으로 각자 필요한 API만 건드리기로 합의해 풀어낸 점이 인상 깊었습니다.',
    ],
  },
  {
    slug: 'whyne',
    ...
    detail:
      // 담당 범위는 git 기록으로 확인 — wines/page.tsx·WineCard·DropDown(공통)·WineTypeDropDown 최초 작성자가 본인
      '사용자들이 다양한 와인 정보를 검색·필터링하며 직접 리뷰를 등록하고 확인하는 리뷰 기반 플랫폼입니다.\n코드잇에서 진행한 중급 팀프로젝트로서 와인 목록 페이지 전체를 담당하였습니다.\n공통 컴포넌트로는 와인카드 컴포넌트, 드롭다운 컴포넌트를 담당하였습니다.',
    card: {
    ...
    retrospect: [
      '프로젝트를 진행하며 배포 환경을 아무도 제대로 모르고 있었다는 게 드러난 적이 있었습니다. 인스턴스를 재시작했더니 접속이 끊겼는데 어디를 봐야 하는지 적어 둔 곳이 없어서 복구까지 오래 걸렸습니다. 그 뒤로는 한 번 겪은 문제는 짧게라도 반드시 문서로 남기게 되었습니다.',
      '프로젝트 기획 중 팀원 한 명이 부트캠프를 그만두면서 R&R이 꼬이기도 했고, 다른 팀원의 UI 컴포넌트 공수가 길어져 스프린트 기간을 맞추지 못하게 되자 제가 그 컴포넌트를 대신 개발해 일정을 맞춘 기억도 있습니다. UI 라이브러리를 쓰지 않고 직접 개발하는 것을 목표로 한 프로젝트였고, 끝까지 완수했습니다.',
    ],
  },
];
